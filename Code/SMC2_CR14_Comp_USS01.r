...
# Read data
DataSet<-read.table("CompactedUSS01.txt",header=TRUE)
Obs<-DataSet[1:321,5]
Obs_Depths<-DataSet[1:321,1]
Obs_Length<-length(Obs_Depths)
Obs_Depth_Step<-Obs_Depths[1:320]-Obs_Depths[2:321]
...
FOL<-99
FileTop<-as.name(paste("../Results/CR12CompactedUSS01NPR",FOL,"/",sep=""))
...
Parameter_Mean<-c(Mean_Beta_0,Mean_Beta_1,Mean_Beta_2,Mean_Delta,Mean_P,Mean_C,Mean_E,Mean_Sigma_1,Mean_Sigma_2,Mean_Sigma_Obs,Mean_D,Mean_S,Mean_Mu_S,Mean_SD_S,Mean_Alpha,Mean_p_0,Mean_Grad)
Parameter_Covariance<-cov.wt(Results[,1:17],wt=Results[,18])$cov
...
SD_1<-sqrt((1/sum(w[[i-1]]))*sum(w[[i-1]]*(c(Int_1)-Mean_1)^2))
SD_2<-sqrt((1/sum(w[[i-1]]))*sum(w[[i-1]]*(c(Int_2)-Mean_2)^2))
SD_T<-sqrt((1/sum(w[[i-1]]))*sum(w[[i-1]]*(c(Int_T)-Mean_T)^2))
...
Parameter_Current<-Results[j,1:17]
...
if(Parameter_Proposed[11]>D_Min&&Parameter_Proposed[11]<D_Max&&Parameter_Proposed[12]>S_Min&&Parameter_Proposed[12]<S_Max&&Parameter_Proposed[3]>0&&Parameter_Proposed[4]>0&&Parameter_Proposed[5]>0&&Parameter_Proposed[7]>0&&Parameter_Proposed[8]>0&&Parameter_Proposed[9]>0&&Parameter_Proposed[10]>0&&Parameter_Proposed[13]>0&&Parameter_Proposed[14]>0&&Parameter_Proposed[15]>0&&Parameter_Proposed[16]>0&&Parameter_Proposed[17]>0){
...
UD<-Obs_Depths+(Parameter_Proposed[17]/(1-Parameter_Proposed[16]))*Obs_Depths^2
w_Proposed<-(dinv.gaussian(-T_Proposed,mu=(UD[1]-UD[Obs_Length])/Parameter_Proposed[13],lambda=(UD[1]-UD[Obs_Length])^2/Parameter_Proposed[14]^2)*dunif(X_1_Proposed,min=x_1_Min,max=x_1_Max)*dunif(X_2_Proposed,min=x_2_Min,max=x_2_Max)*dnorm(Obs[1],mean=Parameter_Proposed[12]*X_1_Proposed+Parameter_Proposed[11],sd=Parameter_Proposed[10])*dnorm(T_Proposed,mean=ACP,sd=2000))/(dnorm(X_1_Proposed,mean=Mean_1,sd=SD_1)*dnorm(X_2_Proposed,mean=Mean_2,sd=SD_2)*dnorm(T_Proposed,mean=Mean_T,sd=SD_T))
...
SMC_Out<-.C("SMC_CR12",as.numeric(UD[LV-1]),as.numeric(UD[LV-1]-UD[LV]),as.numeric(0),as.numeric(0),as.integer(State_Particles),as.numeric(Obs[LV]),Log_M_L=as.numeric(0),as.integer(Random_Seed),X_1=as.numeric(X_1_Proposed),X_2=as.numeric(X_2_Proposed),T=as.numeric(T_Proposed),w=as.numeric(w_Proposed),as.numeric(Parameter_Proposed[1]),as.numeric(Parameter_Proposed[2]),as.numeric(-Parameter_Proposed[3]),as.numeric(Parameter_Proposed[4]),as.numeric(Parameter_Proposed[5]),as.numeric(Parameter_Proposed[6]),as.numeric(Parameter_Proposed[7]),as.numeric(Parameter_Proposed[8]),as.numeric(Parameter_Proposed[9]),as.numeric(Parameter_Proposed[10]),as.numeric(Parameter_Proposed[11]),as.numeric(Parameter_Proposed[12]),as.numeric(Parameter_Proposed[13]),as.numeric(Parameter_Proposed[14]),as.numeric(Parameter_Proposed[15]),A=as.integer(A_Proposed),as.numeric(GPre),as.numeric(GCop),as.numeric(GObl))
...
if(LML_Track!="NaN"){
u<-runif(1)
if(log(u)<LML_Track-Results[j,19]+dmvnorm(Parameter_Current,mean=Parameter_Mean,sigma=Parameter_Covariance,log=TRUE)-dmvnorm(Parameter_Proposed,mean=Parameter_Mean,sigma=Parameter_Covariance,log=TRUE)+dnorm(Parameter_Proposed[1],mean=0.4,sd=0.3,log=TRUE)-dnorm(Parameter_Current[1],mean=0.4,sd=0.3,log=TRUE)+dnorm(Parameter_Proposed[2],mean=0,sd=0.4,log=TRUE)-dnorm(Parameter_Current[2],mean=0,sd=0.4,log=TRUE)+dexp(Parameter_Proposed[3],rate=1/0.5,log=TRUE)-dexp(Parameter_Current[3],rate=1/0.5,log=TRUE)+dexp(Parameter_Proposed[4],rate=1/0.5,log=TRUE)-dexp(Parameter_Current[4],rate=1/0.5,log=TRUE)+dexp(Parameter_Proposed[5],rate=1/0.3,log=TRUE)-dexp(Parameter_Current[5],rate=1/0.3,log=TRUE)+dnorm(Parameter_Proposed[6],mean=0,sd=0.3,log=TRUE)-dnorm(Parameter_Current[6],mean=0,sd=0.3,log=TRUE)+dexp(Parameter_Proposed[7],rate=1/0.3,log=TRUE)-dexp(Parameter_Current[7],rate=1/0.3,log=TRUE)+dexp(Parameter_Proposed[8],rate=1/0.3,log=TRUE)-dexp(Parameter_Current[8],rate=1/0.3,log=TRUE)+dexp(Parameter_Proposed[9],rate=1/0.5,log=TRUE)-dexp(Parameter_Current[9],rate=1/0.5,log=TRUE)+dexp(Parameter_Proposed[10],rate=1/0.1,log=TRUE)-dexp(Parameter_Current[10],rate=1/0.1,log=TRUE)+dgamma(Parameter_Proposed[13],shape=180,scale=1/4000000,log=TRUE)-dgamma(Parameter_Current[13],shape=180,scale=1/4000000,log=TRUE)+dexp(Parameter_Proposed[14],rate=500,log=TRUE)-dexp(Parameter_Current[14],rate=500,log=TRUE)+dgamma(Parameter_Proposed[15],shape=10,scale=2,log=TRUE)-dgamma(Parameter_Current[15],shape=10,scale=2,log=TRUE)+dbeta(Parameter_Proposed[16],shape1=45,shape2=15,log=TRUE)-dbeta(Parameter_Current[16],shape1=45,shape2=15,log=TRUE)+dexp(Parameter_Proposed[17],rate=4000,log=TRUE)-dexp(Parameter_Proposed[17],rate=4000,log=TRUE)) {
Results[j,1:17]<-Parameter_Proposed
...
SMC_Out<-.C("SMC_CR12",as.numeric(UD[i-1]),as.numeric(UD[i-1]-UD[i]),as.numeric(0),as.numeric(0),as.integer(State_Particles),as.numeric(Obs[i]),Log_M_L=as.numeric(0),as.integer(Random_Seed),X_1=as.numeric(X_1[[i-1]][j,]),X_2=as.numeric(X_2[[i-1]][j,]),T=as.numeric(T[[i-1]][j,]),w=as.numeric(w[[i-1]][j,]),as.numeric(Results[j,1]),as.numeric(Results[j,2]),as.numeric(-Results[j,3]),as.numeric(Results[j,4]),as.numeric(Results[j,5]),as.numeric(Results[j,6]),as.numeric(Results[j,7]),as.numeric(Results[j,8]),as.numeric(Results[j,9]),as.numeric(Results[j,10]),as.numeric(Results[j,11]),as.numeric(Results[j,12]),as.numeric(Results[j,13]),as.numeric(Results[j,14]),as.numeric(Results[j,15]),A=as.integer(A[[i-1]][j,]),as.numeric(GPre),as.numeric(GCop),as.numeric(GObl))
